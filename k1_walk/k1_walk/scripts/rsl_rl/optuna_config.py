...
class OptunaConfig:
    ...
    # スタディ設定
    study_name: str = "k1_walk_rsl_rl_optimization"
    storage_path: str = "optuna_studies/study.db"
    n_trials: int = 40
    n_startup_trials: int = 10  # TPEサンプラーの初期ランダムトライアル数

    # 学習設定
    base_command: str = "bash train.sh"
    task: str = "K1-Walk-Train-rough"
    num_envs: int = 4096
    max_iterations: int = 1000
    extra_args: str = ""  # train.shに渡す追加引数

    # 評価設定
    eval_metric: str = "Episode/mean_length"  # 平均エピソード長を最大化
...
class RewardSearchSpace:
    ...
    # タスク報酬
    track_lin_vel_xy_exp: Tuple[float, float, bool] = (2.0, 20.0, False)
    track_ang_vel_z_exp: Tuple[float, float, bool] = (1.5, 15.0, False)
    feet_height_bezier: Tuple[float, float, bool] = (5.0, 30.0, False)
