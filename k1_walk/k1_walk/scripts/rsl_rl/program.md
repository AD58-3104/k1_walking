...
**What you CAN do:**
- Modify `k1_walk_env_cfg.py` — あなたが変更可能なファイルの1つです。これには、rlの報酬の重みやDRの設定値など、学習時の環境設定に関するハイパーパラメータが記載してあります。このファイルだけ編集できます。
- Modify `rsl_rl_ppo_cfg.py` - これには、ppoの学習時のハイパーパラメータを記載します。
...
1. Tune `k1_walk_env_cfg.py` and `rsl_rl_ppo_cfg.py` with an experimental idea by directly hacking the code.
2. git commit
