...
class OptunaOptimizer:
    """RSL-RL報酬重み最適化のメインクラス"""

    def __init__(self, config: OptunaConfig):
        self.config = config
        self.script_dir = Path(__file__).parent.resolve()
        self.log_root = self.script_dir / "logs" / "rsl_rl"
    ...
    def optimize(self) -> None:
        ...
        print(f"\n{'='*60}")
        print(f"Starting Optuna Optimization (RSL-RL)")
        ...
        print(f"Direction: {self.config.eval_metric_direction}")
    ...
    def _save_results(self, study: optuna.Study) -> None:
        ...
        results = {
            ...
            "config": {
                ...
                "eval_metric_direction": self.config.eval_metric_direction,
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Optuna による RSL-RL 報酬重み最適化"
    )
    parser.add_argument(
        "--study-name",
        type=str,
        default="k1_walk_rsl_rl_optimization",
        help="Optunaスタディ名"
    )
    parser.add_argument(
        "--n-trials",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="評価メトリクス（デフォルト: Episode/mean_length）"
    )
    parser.add_argument(
        "--direction",
        type=str,
...
def main():
    ...
    # コマンドライン引数で評価メトリクスを上書き
    if args.eval_metric:
        config.eval_metric = args.eval_metric
    if args.direction:
        config.eval_metric_direction = args.direction
